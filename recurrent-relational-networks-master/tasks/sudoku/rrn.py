...
from tasks.sudoku.data_test import queens
...
class SudokuRecurrentRelationalNet(Model):
    ...
    def __init__(self, is_testing):
        ...
        with tf.Graph().as_default(), tf.device('/cpu:0'):
            ...
            self.train, self.valid, self.test = self.encode_data(queens())
            ...
            #total_box = 81      #for sudoku
            #one_side = 9
            total_box = 16       #for n-queens
            r_or_c = 4 

            if self.edges == 'sudoku':
                edges = self.sudoku_edges()
            elif self.edges == 'full':
                edges = [(i, j) for i in range(total_box) for j in range(total_box) if not i == j]
            else:
                raise ValueError('edges must be sudoku or full')

            edge_indices = tf.constant([(i + (b * total_box), j + (b * total_box)) for b in range(self.batch_size) for i, j in edges], tf.int32)
            n_edges = tf.shape(edge_indices)[0]
            edge_features = tf.zeros((n_edges, 1), tf.float32)
            positions = tf.constant([[(i, j) for i in range(r_or_c) for j in range(r_or_c)] for b in range(self.batch_size)], tf.int32)  # (bs, 81, 2)
            print ("positions", positions)
            rows = layers.embed_sequence(positions[:, :, 0], r_or_c, self.emb_size, scope='row-embeddings', unique=True)  # bs, 81, emb_size
            print ("rows", rows)
            cols = layers.embed_sequence(positions[:, :, 1], r_or_c, self.emb_size, scope='cols-embeddings', unique=True)  # bs, 81, emb_size
            print ("cols", cols)
            ...
            with tf.variable_scope(tf.get_variable_scope()):
                for device_nr, device in enumerate(self.devices):
                    with tf.device('/cpu:0'):
                        ...
                        total_options = 2        # n-queens
                        #total_options = 10      # sudoku
                        
                        print ("Quizzes:", quizzes, "Answers:", answers)    
                        x = layers.embed_sequence(quizzes, total_options, self.emb_size, scope='nr-embeddings', unique=True)  # bs, 81, emb_size
                        print ("after layer embedding sequence",x)
                        x = tf.concat([x, rows, cols], axis=2)
                        print ("after concat embedding sequence",x)
                        x = tf.reshape(x, (-1, 3 * self.emb_size))
                        print ("after reshape embedding sequence",x)
                        ...
                        with tf.variable_scope('steps'):
                            ...
                            for step in range(self.n_steps):
                                ...
                                with tf.variable_scope('graph-sum'):
                                    out = tf.reshape(layers.fully_connected(x, num_outputs=10, activation_fn=None), (-1, total1, total2))
                                    print("out:",out)
